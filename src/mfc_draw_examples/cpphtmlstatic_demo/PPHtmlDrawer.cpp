...
void CPPHtmlDrawer::ReplaceSpecChars()
{
	...
	if (!m_bEnableEscapeSequences)
	{
		...
		//RUS: Óäàëÿåì ñïåöèàëüíûå ñèìâîëû
	...
	else
	{
		...
		//RUS: Çàìåíÿåì ñïåöèàëüíûå ñèìâîëû HTML òýãàìè
...
void CPPHtmlDrawer::DrawHtml (LPSIZE lpSize, LPCRECT lpRect)
{
	...
	//RUS: Îãðàíè÷èâàþùåé ïðÿìîóãîëüíèê äëÿ òåêóùåé îáëàñòè âûâîäà
	RECT rcArea;
	...
	if (MODE_FIRSTPASS == m_nNumPass)
	{
		...
		//RUS: Â ðåæèìå ïîäãîòîâêè î÷èùàåì âñïîìîãàòåëüíûå òàáëèöû 
		m_arrLinks.clear();
//		m_arrTable.clear();
		m_arrHtmlLine.clear();
//		m_arrTableSizes.clear();
	...
	//RUS: Çàïîìèíàåì òåêóùèå íàñòðîéêè êîíòåêñò óñòðîéñòâà
...
//			strText = _T("<body>") + strText + _T("</body>");
	...
	//RUS: Âîññòàíàâëèâàåì íàñòðîêè êîíòåêñòà óñòðîéñòâà
...
//		sc.nWidth = 0;
		...
		//RUS: Ñîçäàåì øàáëîí ïóñòîé òàáëèöû
		vecRow rows;
		...
		//ENG: Add a new table
		//RUS: Äîáàâëÿåì íîâóþ òàáëèöó
		m_arrTables.push_back(st);
	} //if

	//ENG: Gets an info about a current table
	//RUS: Âçÿòü èíôîðìàöèþ î òåêóùåé òàáëèöå 
	...
	//RUS: Ïðèìåíÿåì ñòèëè òàáëèöû (òýã <table>)
	...
	//RUS: Ïðîïóñêàåì òýã íà÷àëà ÿ÷åéêè è ïîëó÷àåì ñòðîêó ñâîéñòâ òýãà
	int nIndex = 0;
	CPPString sTag;
	...
	if (MODE_FIRSTPASS != m_nNumPass)
	{
		...
		//RUS: Ïîëó÷àåì ðåàëüíûå ðàçìåðû òàáëèöû
		...
		//RUS: Âûðàâíèâàíèå òàáëèöû ïî ãîðèçîíòàëè
		int nRealWidth = lpRect->right - lpRect->left;

		if (nWidthTable < nRealWidth)
		{
			//RUS: Ïîïûòàåìñÿ ðàñòÿíóòü òàáëèöó íà âñþ äîñòóïíóþ îáëàñòü
...
//		size.cx = GetTableWidth(strTable, 0, size.cx, TRUE);
	...
	//RUS: Ñîõðàíÿåì òåêóùóþ òàáëèöó
	m_arrTables [nIndexTable] = cur_table;

	//ENG: Restore styles before <table> tag
	//RUS: Âîññòàíàâëèâàåì ñòèëè äî òýãà <table>
	m_defStyle.strTag = _T("table");
...
void CPPHtmlDrawer::DrawHtmlTableRow(CPPString & sRow, LPCRECT lpRect, STRUCT_TABLE & st, int nRow)
{
	...
	//RUS: Ïðèìåíÿåì ñòèëè ñòðîêè (òýã <tr>)
	...
	while (nIndex < sRow.GetLength())
	{
		...
		if (nNewCell < nEndRow)
		{
			...
			if (MODE_FIRSTPASS != m_nNumPass)
			{
				...
				//RUS: Ïîëó÷àåì ðåàëüíûé ïðÿìîóãîëüíèê äëÿ âûâîäà ÿ÷åéêè
				rcCell = *lpRect;
				...
				//RUS: cellspacing - îòñòóï îò êîíòóðà òàáëèöû äî ÿ÷åéêè 
//				rcCell.left += m_defStyle.nPadding;
//				rcCell.top += m_defStyle.nPadding;
...
//				rcCell.bottom -= m_defStyle.nPadding;
			...
			if (MODE_DRAW != m_nNumPass)
			{
				...
				//RUS: Äîáàâëÿåì îòñòóï ÿ÷åéêè îò
...
//				sc.szCell.cy += m_defStyle.nPadding + m_defStyle.nPadding;
				...
				//RUS: Çàïîìèíàåì îáúåäèíåííûå ÿ÷åéêè
	...
	//RUS: Âîññòàíàâëèâàåì ñòèëè äî òýãà <tr>
	m_defStyle.strTag = _T("tr");
	if (StoreRestoreStyle(TRUE))
		UpdateContext();
...
void CPPHtmlDrawer::DrawHtmlTableCell(CPPString & sCell, LPCRECT lpRect, STRUCT_CELL & sc)
{
	...
	//RUS: Ïðèìåíÿåì ñòèëè ÿ÷åéêè (òýã <td>)
	...
	if (MODE_FIRSTPASS == m_nNumPass)
	{
		...
		//RUS: Ñîõðàíÿåì èíôîðìàöèþ îá îáúåäèíåíèè ÿ÷ååê
		sc.nColSpan = szSpan.cx;
		sc.nRowSpan = szSpan.cy;
		//ENG: Stores an info about the recommended cell sizes
		//RUS: Ñîõðàíÿåì èíôîðìàöèþ îá ðåêîìåíäîâàííûõ ðàçìåðàõ ÿ÷åéêè
...
//		sc.bHeightPercent = m_defStyle.bCellHeightPercent;
		//ENG: 
		//RUS: Åñëè óêàçàíû ìèíèìàëüíûå ðaçìåðû ÿ÷åéêè, òî óñòàíîâèòü èõ êàê íà÷àëüíûå
...
//		sc.szText.cy = m_defStyle.nCellHeight;
	...
	else if (MODE_DRAW == m_nNumPass)
	{
		...
		//RUS: cellpadding - îòñòóï îò êîíòóðà ÿ÷åéêè, äî òåêñòà âíóòðè åå
	...
	while(nIndex < sCell.GetLength())
	{
		...
		//RUS: Ïîëó÷àåì èíäåêñ áëèæàéøåãî òýãà
		...
		if (nNearlyTag > nIndex)
		{
			...
			//RUS: Åñëè ìåæäó ïîñëåäíèì èíäåêñîì è òåêóùèì èíäåêñîì ñóùåñòâóåò òåêñò
		...
		if (MODE_DRAW != m_nNumPass)
		{
			...
			//RUS: Íà ïåðâîì è âòîðîì ïðîõîäàõ âû÷èñëÿåì ðàçìåðû ÿ÷åéêè.
	...
	if (MODE_DRAW != m_nNumPass)
	{
		...
		//RUS: Äîáàâëÿåì îòñòóïû òåêñòà îò ãðàíèö ÿ÷åéêè
	...
	//RUS: Âîîñòàíàâëèâàåì ñòèëè, êîòîðûå áûëè äî òýãà <td>
	m_defStyle.strTag = _T("td");
	if (StoreRestoreStyle(TRUE))
		UpdateContext();
}
...
SIZE CPPHtmlDrawer::DrawHtmlString (CPPString & sHtml, LPCRECT lpRect)
{
	...
	COLORREF clrShadow = m_bIsEnable ? m_crShadow : GetColorByName(TEXT(""));

	//ENG: For any string we are add a <body> tag as wrapper
	//RUS: Äëÿ ëþáîé ñòðîêè äîáàâëÿåì òýã <body>
	sHtml = _T("<body>") + sHtml;
	sHtml += _T("</body>");

	//ENG: Bounding rectangle for a full text
	//RUS: Îãðàíè÷èâàþùèé ïðÿìîóãîëüíèê äëÿ âûâîäà âñåãî òåêñòà
...
//	m_szOutput = CSize(0, 0);
	...
	//RUS: Åñëè òåêñòà äëÿ âûâîäà íåò
//	if (str.IsEmpty())
//	{
//		szTextArea.cx = szTextArea.cy = 0;
//		return;
...
//	pt.y = lpRect->top;
	...
	//RUS: Èíèöèàëèçàöèÿ íîâîé ñòðîêè
	...
	while (i < sHtml.GetLength())
	{
		...
		//RUS: Ïîèñê ïåðâîãî òýãà
		sText = SearchNextTag(sHtml, sTag, i);
		sProperties = SplitTag(sTag);

		//ENG: Before a tag was exist a text
		//RUS: Ïåðåä òýãîì åñòü òåêñò äëÿ âûâîäà
		if (!sText.IsEmpty())
		{
			//ENG: Transform text
			//RUS: Ïðåîáðàçóåì òåêñò
			switch (m_defStyle.nTextTransform)
			{
			case TEXT_TRANSFORM_UPPERCASE:
				//ENG: All chars make upper
				//RUS: Âñå ñèìâîëû ïåðåâîäèì â âåðõíèé ðåãèñòð
				sText.MakeUpper();
				break;
			case TEXT_TRANSFORM_LOWERCASE:
				//ENG: All chars make lower
				//RUS: Âñå ñèìâîëû ïåðåâîäèì â íèæíèé ðåãèñòð
				sText.MakeLower();
				break;
			case TEXT_TRANSFORM_CAPITALIZE:
				//ENG: Each first char of a word to upper
				//RUS: Êàäûé ïåðâûé ñèìâîë ñëîâà â âåðõíèé ðåãèñòð, îñòàëüíûå â íèæíèé
				sText.MakeLower();
			...
			//RUS: Çàöèêëèâàåì äî òåõ ïîð, ïîêà íå áóäåò âûâåäåí âåñü òåêñò
			while (!sText.IsEmpty())
			{
				//ENG: Reset an additional interval for space chars
				//RUS: Ñáðîñ äîïîëíèòåëüíîãî èíòåðâàëà ìåæäó ñëîâàìè
				::SetTextJustification(m_hDC, 0, 0);

				//ENG: Gets a size a output text
				//RUS: Ïîëó÷àåì ðàçìåð âûâîäèìîãî òåêñòà
				::GetTextExtentPoint32(m_hDC, sText, sText.GetLength(), &sz);

				//ENG: Gets a real top coordinate to output with vertical alignment
				//RUS: Ïîëó÷àåì ðåàëüíóþ êîîäèíàòó âåðõà âûâîäà ñ ó÷åòîì âåðòèêàëüíîãî âûðàâíèâàíèÿ
				...
				if (MODE_DRAW == m_nNumPass)
				{
					if (sz.cx)
					{
						if ((0 == (nRealWidth - sz.cx)) && (_T(' ') == sTemp.GetAt(sTemp.GetLength() - 1)))
						{
							...
							//RUS: Åñëè ýòî ïîñëåäíèé âûâîä â ñòðîêå, òî óáèðàåì ïðîáåëû ñïðàâà
							sTemp.TrimRight();
						...
						//RUS: Ñîõðàíÿêì òåêóùóþ îáëàñòü êàê îáëàñòü ãèïåðëèíêà åñëè îí óñòàíîâëåí
						...
						//RUS: Åñëè óñòàíîâëåí ñòèëü overline, òî ðèñóåì ëèíèþ íàä òåêñòîì
				...
				else
				{
					...
					//RUS: Ñîõðàíÿåì ïîñëåäíåå ãîðèçîíòàëüíîå âûðàâíèâàíèå
				...
				//RUS: Ïåðåìåùàåìñÿ ñïðàâà îò âûâåäåííîãî òåêñòà
				ptOutput.x += sz.cx;
				if (!sText.IsEmpty())
				{
					//ENG: Not all text was printed (cause text wrap) 
					//RUS: Íå âñÿ ñòðîêà åùå âûâåëàñü (â ñëó÷àå ïåðåíîñà òåêñòà)
				...
				if (!m_defStyle.strTag.IsEmpty())
				{
					...
					if (StoreRestoreStyle(bCloseTag))
					{
						...
						if (!bCloseTag)
						{
									...
									if (!sParameter.IsEmpty())
									{
										...
										else if (sParameter == _T("color"))
										{
											if (m_bIsEnable)
												...
												m_defStyle.crText = GetColorByName(TEXT(""));
										}
							...
							case TAG_HYPERLINK:
								...
								//RUS: Ïîëó÷àåì èíäåêñ òåêóùåé ãèïåðññûëêè
								m_nCurIndexLink ++;
								//ENG: If a mouse over this link
								//RUS: Åñëè ìûø íàä ýòèì òýãîì
							...
							//RUS: Îáíîâëåíèå êîíòåêñòà óñòðîéñòâà
							UpdateContext();
						} //if
					}
					else 
					{
						...
						case TAG_HLINE:
							...
							//RUS: Ðèñîâàíèå ãîðèçîíòàëüíîé ëèíèè
							csTemp = m_defStyle;
								...
								if (!sParameter.IsEmpty())
								{
									...
									else if (sParameter == _T("color"))
									{
										if (m_bIsEnable)
											...
											csTemp.crText = GetColorByName(TEXT(""));
									}
								} //if
							} //while
						...
						case TAG_BITMAP:
							...
							if (si.nIdRes || si.nIdDll || si.nHandle || !si.strSrcFile.IsEmpty())
							{
								...
								else if (si.nHandle)
								{
									...
									//RUS: Åñëè óêàçàí äåñêðèïòîð èçîáðàæåíèÿ, òî çàïðåùàåì óäàëåíèå
									bAutoDelete = FALSE;
								} //if
								
								//ENG: If a handle of an image was retrieved
								//RUS: Åñëè äåñêðèïòîð èçîáðàæåíèÿ ïîëó÷åí
								if (NULL != hBitmap)
								{
									//ENG: Image with shadow or not?
									//RUS: Èçîáðàæåíèå ñ òåíüþ èëè íåò
									bShadow = IsImageWithShadow(si);
									
									//ENG: Retrieves an original size of an image
									//RUS: Ïîëó÷àåì îðèãèíàëüíûé ðàçìåð èçîáðàæåíèÿ
									m_drawmanager.GetSizeOfBitmap(hBitmap, &sz);
									
									//ENG: Retrieves an output size
									//RUS: Ïîëó÷àåì ðàçìåðû äëÿ ðèñîâàíèÿ
									...
									//RUS: Åñëè òåíü äîñòóïíà, òî óñòàíàâëèâàåì ðåàëüíûé ðàçìåð
									...
									//RUS: Çàïîìèíàåì ïîñëåäíåå ãîðèçîíòàëüíîå âûðàâíèâàíèå
									if (MODE_FIRSTPASS == m_nNumPass) 
										m_hline.nHorzAlign = m_defStyle.nHorzAlign;
									
									//ENG: Retrieves a vertical coordinates of drawing area
									//RUS: Ïîëó÷àåì âåðòèêàëüíóþ êîîðäèíàòó îáëàñòè ðèñîâàíèÿ
									y = VerticalAlignImage(ptOutput.y, si.nHeight);
									
									//ENG: If an image is exist and not prepare mode
									//RUS: Åñëè èçîáðàæåíèå äîñòóïíî è íå óñòàíîâëåí ðåæèì ïîäãîòîâêè
									if (si.nWidth && si.nHeight && (MODE_DRAW == m_nNumPass))
									{
										//ENG: Add an output area to hyperlink list if needed
										//RUS: Åñëè íåîáõîäèìî äîáàâëÿåì îáëàñòü âûâîäà â ñïèñîê ãèïåðññûëîê
										StoreHyperlinkArea(ptOutput.x, y, ptOutput.x + sz.cx, y + sz.cy);
										
										//ENG: If a mouse over an image then applies a hot styles
										//RUS: Åñëè ìûøü íàä èçîáðàæåíèåì, òî ïðèìåíÿåì ñîîòâåñòâóþùèå ñòèëè
									...
									//RUS: Ïåðåìåùàåìñÿ ñïðàâà îò âûâåäåííîãî èçîáðàæåíèÿ
									ptOutput.x += sz.cx; //si.nWidth;
									
									//ENG: If needed delete a handle of an image
									//RUS: Åñëè íåîáõîäèìî óäàëÿåì äåñêðèïòîð èçîáðàæåíèÿ
									if (bAutoDelete)
						...
						case TAG_ICON:
							...
							if (si.nIdRes || si.nIdDll || si.nHandle || !si.strSrcFile.IsEmpty())
							{
								...
								//RUS: Ïîëó÷àåì òðåáóåìûé ðàçìåð èêîíêè
								sz.cx = si.nWidth;
								sz.cy = si.nHeight;
						...
						case TAG_IMAGELIST:
							...
							if (si.nIdRes || si.nIdDll || si.nHandle || !si.strSrcFile.IsEmpty())
							{
								...
								if (NULL != hBitmap)
								{
									...
									//RUS: Ñîçäàåì íåçàäàííûå ðàçìåðû
									if (!si.cx && !si.cy)
									...
									//RUS: Ïîëó÷àåì ìàêñèìàëüíîå ÷èñëî êîëîíîê è ñòðîê èçîáðàæåíèé íà áèòìàïêå
									nMaxCol = sz.cx / si.cx;
									nMaxRow = sz.cy / si.cy;
									...
									//RUS: Åñëè óêàçàííûé èíäåêñ èçîáðàæåíèÿ äîïóñòèì
...
int CPPHtmlDrawer::InitNewLine(int x)
{
	if (MODE_FIRSTPASS == m_nNumPass)
	{
		...
		//RUS: Ñîçäàíèå íîâîé ëèíèè ñ ïàðàìåòðàìè ïî-óìîë÷àíèþ
	...
	else if (MODE_DRAW == m_nNumPass)
	{
		...
		//RUS: Ïîëó÷àåì äàííûå ïåðâîé ñòðîêè è ïðåîáðàçóåì ïðîöåíòíóþ øèðèíó â ðåàëüíóþ
		...
		//RUS: Êîîðäèíàòà íà÷àëà âûâîäà ñ ó÷åòîì âûðàâíèâàíèÿ
...
void CPPHtmlDrawer::Draw(HDC hDC, LPCTSTR lpszHtml, LPPOINT lpPoint)
{
	...
	//RUS: Âûâîä ïîäãîòîâëåííîãî òåêñòà
...
void CPPHtmlDrawer::PrepareOutput(HDC hDC, LPCTSTR lpszHtml, LPSIZE lpSize)
{
	...
	//RUS: Êîïèðîâàíèå íà÷àëüíûõ ïàðàìåòðîâ
	m_hDC = hDC;
...
//	if (m_bIsTextWrapEnabled)
	...
	//RUS: Åñëè ïîäãîòîâëåííûé òåêñò íå ïóñòîé, òî âûõîä
	if (!m_csHtmlText.IsEmpty())
	{
		//ENG: Sets a prepare mode
		//RUS: Óñòàíàâëèâàåì ðåæèì ïîäãîòîâêè
		m_nNumPass = MODE_FIRSTPASS;

		m_arrTables.clear();

		//ENG: Prepares to real output
		//RUS: Ïîäãîòîâêà ê ðåàëüíîìó âûâîäó
...
void CPPHtmlDrawer::DrawPreparedOutput(HDC hDC, LPCRECT lpRect)
{
	...
	//RUS: Åñëè ïîäãîòîâëåííûé òåêñò ïóñòîé, òî âûõîä
	if (m_csHtmlText.IsEmpty())
		return;
	...
	//RUS: Óñòàíàâëèâàåì ðåæèì âûâîäà
...
//		rect.right = rect.left + m_nMaxWidth;
	...
	//RUS: Âûâîä ïîäãîòîâëåííîé ñòðîêè
	DrawHtml(&size, &rect);
...
void CPPHtmlDrawer::SetImageList(HBITMAP hBitmap, int cx, int cy, int nCount, COLORREF crMask /* = RGB(255, 0, 255) */)
{
	...
	//RUS: Óäàëÿåì ïðåäûäóùèé ñïèñîê èçîáðàæåíèé
	if (NULL != m_hImageList)
		::DeleteObject(m_hImageList);

	//ENG: If don't need to create a new image list
	//RUS: Åñëè íå íóæíî ñîçäàâàòü íîâûé ñïèñîê èçîáðàæåíèé
	if (NULL == hBitmap)
		return;
			...
			if (0 != chSymbol)
			{
				...
				else if (strName == _T("border-color"))
				{
					if (m_bIsEnable)
						...
						cs.crBorderLight = GetColorByName(TEXT(""));
...
CPPString CPPHtmlDrawer::SplitTag(CPPString & sTag)
{
	...
	if (tch != _T('\0'))
	{
		...
		//RUS: Ðàçäåëèòåëü íàéäåí. Ðàçäåëÿåì òåëî òýãà íà èìÿ è ïàðàìåòðû
...
SIZE CPPHtmlDrawer::GetTableDimensions(CPPString & sTable)
{
	...
	//RUS: Ðàçìåðû òàáëèöû ïî óìîë÷àíèþ
	SIZE szTable = {0, 0};
	int nIndex = 0;
	int nCol = 0;
	while (nIndex < sTable.GetLength())
	{
		...
		if (SearchTag(sTable, nIndex, _T("tr")))
		{
			...
			do 
			{
				...
				//RUS: Èùåì êîíåö ñòðîêè èëè íà÷àëî ÿ÷åéêè
				...
				if (nNewCell < nEndRow)
				{
					...
					//RUS: Óâåëè÷èâàåì êîëè÷åñòâî ÿ÷ååê â ñòðîêå
					nCol += szSpan.cx;

					//ENG: Jump to end of the cell
					//RUS: Ïåðåõîäèì íà êîíåö ÿ÷åéêè
...
SIZE CPPHtmlDrawer::AnalyseCellParam(CPPString & sProperties, _STRUCT_CHANGESTYLE & cs, BOOL bTable)
{
	...
	while (i < sProperties.GetLength())
	{
		...
		//RUS: Îáðàáàòûâàåì ñïåöèôè÷åñêèå äëÿ òýãà <table> ïàðàìåòðû
		if(bTable)
		{
		...
		else if (sParameter == _T("bordercolordark"))
		{
			if (m_bIsEnable)
				...
				cs.crBorderDark = GetColorByName(TEXT(""));
		}
	...
	//ENG: 
	//RUS: Äëÿ ÿ÷ååê øèðèíà âñåãäà ðàâíà 1
...
CPPString CPPHtmlDrawer::GetWordWrap(CPPString & str, int nMaxSize, int & nRealSize)
{
	...
	if (0 == nLastIndex)
	{
		...
		{
			//RUS: Ðàçðûâîâ â ñòðîêå íå îáíàðóæåíî, ïîýòîìó áóäåì ðàçáèâàòü ñòðîêó 
			//     ïî ñèìâîëàì, à íå ïî ñëîâàì
			sz.cx = 0;
			int i;
			...
			//RUS: Íåâîçìîæíî ðàçáèòü ñòðîêó, âûâîäèì öåëèêîì
			sResult = str;
			str.Empty();
		}
		else
		{
			//RUS: Â îòñòàâøååñÿ ìåñòî òåêóùåé ñòðîêè íå âëàçèò íè îäíîãî ñëîâà
			sz.cx = 0;
		} //if
	}
	else 
